 ...
 * Patchs by Christophe Desguez - http://eidolon-labs.com
 * @version 2.1.1
 * @date August 26, 2011
    ...
    $.fn.extend({
        infiniteCarousel:function (options) {
            ...
            return this.each(function () {
                ...
                var firstImg = $('img:first', $obj);
                var imgHeight = firstImg.outerHeight();
                var imgWidth = firstImg.outerWidth();
                var padded = $("<img>").css({height:0, width:0, border:'none', margin:o.padding});
                imgHeight += padded.outerHeight(true);
                imgWidth += padded.outerWidth(true);
                ...
                $obj.css({'position':'relative', 'overflow':'hidden'})
                    .width(imgWidth * o.inView)
                    .height(imgHeight); //,'overflow':'hidden'
                $li.css({'list-style':'none', 'margin':'0', 'padding':'0', 'position':'relative'})
                    .width(imgWidth * numImages);
                $('li', $obj).css({'display':'inline', 'float':'left', 'padding':o.padding});

                // Move rightmost image over to the left
                $('li:last', $obj).prependTo($li);
                $li.css('left', -imgWidth + 'px').width(9999);
                ...
                for (i = 1; i <= o.inView; i++) {
                    ...
                    $('#textholder' + randID + '_' + i).css({
                        'left':(i - 1) * imgWidth,
                        'margin-left':padded.css('margin-left') + containerPaddingLeft,
                        'margin-right':padded.css('margin-right')
                    });
                    $('#textholder' + randID + '_' + i)
                        .height(imgHeight * o.textholderHeight)
                        .css({'backgroundColor':'#FFF', 'opacity':o.textholderOpacity});
                ...
                function moveLeft(dist) {
                    ...
                    } // If running a continuous show with no display time, fist clear the interval. Then below, recursively call moveLeft
                    ...
                    $li.animate({left:-imgWidth * (dist + 1)}, o.transitionSpeed, o.easeLeft, function () { // Animate the entire list to the left
                ...
                function moveRight(dist) {
                    ...
                    $li.css('left', -(imgWidth * (dist + 1)))
                        .animate({left:-imgWidth}, o.transitionSpeed, o.easeRight, function () {
                ...
                // register a controler on our object so that we can stop and start it externally
                var controler = {
                    options:o,
                    stop:forcePause,
                    start:forceStart,
                    ...
                    remove:function () {
                        this.stop();
                        $(".textholder", $obj).remove();

                        $("*", $obj).removeAttr("style");
                        $obj.removeAttr("style");
                        /*
                         * Beware : removeAttr is broken in jQuery 1.6 > 1.6.2
                         * (doesn't work in Chrome) !!
                         * Use these workarounds instead if you have jQuery 1.6
                         *
                         $.each($("*", $obj).get(), function(i, elt) {
                         elt.setAttribute("style", "");
                         elt.removeAttribute("style");
                         });
                         $obj.get(0).setAttribute("style", "");
                         $obj.get(0).removeAttribute("style");
                         */

                        $obj.removeData("carousel");
                    }
                };

                $obj.data("carousel", controler);
            });
        }
    });
})(jQuery);